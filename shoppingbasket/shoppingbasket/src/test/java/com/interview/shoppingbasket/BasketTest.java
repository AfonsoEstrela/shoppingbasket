...
public class BasketTest {
    ...
    void consolidateBasketTest() {
        ...
        basket.add("productCode", "myProduct3", 10);
        basket.add("productCode2", "myProduct3", 20);
        basket.add("productCode", "myProduct3", 10);

        basket.consolidateItems();

        assertEquals(basket.getItems().size(),2);
        assertEquals(basket.getItems().get(0).getProductCode(), "productCode");
        assertEquals(basket.getItems().get(0).getQuantity(), 40);
        assertEquals(basket.getItems().get(1).getProductCode(), "productCode2");
        assertEquals(basket.getItems().get(1).getQuantity(), 30);
